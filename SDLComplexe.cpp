...
void SDLComplexe::drawAxis(){
	...
	// === graduations
	for(int i = 1 ; i <= graduation_count*2 + 1 ; i++) {
		if(i == graduation_count + 1) continue;
		this->line(i*int(W()/(graduation_count*2 + 2)), int(H()/2) - 5, i*int(W()/(graduation_count*2 + 2)), int(H()/2) + 5); // horizontal
		this->line(int(W()/2) - 5, i*int(H()/(graduation_count*2 + 2)), int(W()/2) + 5, i*int(H()/(graduation_count*2 + 2))); // vertical
	}
