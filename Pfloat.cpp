...
Pfloat Pfloat::pi( ){
	if(STANDARD_PRECISION <= 1000) return Pfloat("3.");

	// load pi from file
	char * 	pi_str = (char *)malloc(1000005*sizeof(char));

    FILE * 	f = fopen("milion_digit_pi.txt", "r");   
	if (f == NULL) throw std::runtime_error("no such file named 'milion_digit_pi.txt' in the current directory");

	char 	c;
	int 	count = 0;

	while (!feof(f) && (c = fgetc(f)) > 0 && count < STANDARD_PRECISION + 2) pi_str[count++] = c;	
